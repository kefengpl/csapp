...
/* 
 * NOTE: in twpo's complement: ~x = -x - 1 
...
//4
/* 思路是 除了 INT_MIN 和 0(这二者取负号结果是它本身)，其他数字取负号，其符号都会变为相反。
...
/*
 *  n 个比特位能表示的数值范围是 -2^{n - 1} ~ 2{n - 1} - 1
 *  1 bits -> -1 ~ 0; 2 bits -> -2 ~ 1; 3 bits -> -4 ~ 3; 4 bits -> -8 ~ 7
 ...
 *  Max ops: 90
 *  Rating: 4
 * 正确答案：找最高位的1即可。对于负数要先取反。
 * 需要使用二分法。最高位的1的位索引(0--31) + 2.
 */
