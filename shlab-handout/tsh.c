...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        // 在原来的 blockset 基础上，增加阻塞 SIGCHLD
 ...
 * @note 你没有对 jobs 结构做出保护。addjobs/deletejobs 都屏蔽了信号，这里或许你也需要屏蔽讯号
 * @note SIGCHLD 信号是否需要解除？
 */
void waitfg(pid_t pid)
{
...
void sigchld_handler(int sig) 
{
    ...
    sigset_t all_mask, prev_mask;
    sigfillset(&all_mask);
    //! \bug 为何 SIGSTP 的子进程会被回收？因为 WUNTRACED 的作用是记录 stopped 子进程的状态，此时 while 循环也能进入
...
void sigtstp_handler(int sig) 
{
    ...
    fg_pid = fgpid(jobs);
    if (fg_pid != 0) {
        //! \bug SIGTSTP 会导致子进程进入 STOPPED 状态，而 Child stopped 会给父进程发送 SIGCHLD 信号。
        kill(-fg_pid, SIGTSTP);
    }
    sigprocmask(SIG_SETMASK, &prev_mask, NULL);
    return;
...
pid_t Fork(void) {
    pid_t pid = 0;
    //! \bug 当年你把代码写成了 if ((pid = fork() < 0))，由于 fork() < 0 的运算优先级更高，
    //! \bug 而 fork 父进程返回子进程 pid (大于0)，所以 (fork < 0) == false；子进程返回 0, (fork < 0) == false，
    //! \bug 所以 Fork 返回的值在父子进程都是 0，于是，父进程在 Fork 后的代码是无法正常跳转执行的。 
    if ((pid = fork()) < 0) 
