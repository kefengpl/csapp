...
```
随后进入最后一段循环代码：访问数组b。
- 循环伊始，%rbx == arr[0] == 603320，对应数组 b 的元素是 `0x00603320: bb 01 00 00 06 00 00 00 10 33 60 00 00 00 00 00`(1)，取出该元素后 8 个字节 603310，对应数组 b 的元素是 `0x00603310: dd 01 00 00 05 00 00 00 00 33 60 00 00 00 00 00`(2)，需要满足(1)的前4个字节对应数值`01bb` >= (2)的前4个字节对应数值`01dd`。
- 随后，%rbx更新为(1)的后8个字节 603310 ，对应数组 b 的元素是`0x00603310: dd 01 00 00 05 00 00 00 00 33 60 00 00 00 00 00`(2)，取出该元素后 8 个字节 603300，对应数组 b 的元素是 `0x00603300: b3 02 00 00 04 00 00 00 f0 32 60 00 00 00 00 00`(3)，需要满足(2)的前4个字节对应数值`01dd` >= (3)的前4个字节对应数值`02b3`。
