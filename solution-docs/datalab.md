...
6. 舍入(Nearest Even)
- 十进制： **Nearest Even 表示四舍六入五留双，如果被舍弃的部分形如50000...(5后面全是0，亦即5在数字的末尾)，那么需要保证舍入后最低位是偶数。** 比如 1.5舍弃小数部分，5进一位得到2，所以舍入结果是2；7.895 舍入到百分位，5进一位得到7.90(0是偶数)；7.885 舍入到百分位，舍弃5得到7.88(百分位的8是偶数)；2.451 舍入到十分位，由于被舍弃部分是 51，5后面不全是0，不适用Nearest Even规则，>50所以直接进位，舍入后结果是 2.5；2.45 舍入到十分位，由于被舍弃部分是 5，5在数字末尾，所以适用Nearest Even规则，不进位才能保证十分位是偶数，所以舍入后结果是 2.4。
